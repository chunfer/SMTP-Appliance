...
class FileDev(object):
	...
	def FileProcess(self, audiofile='tmp.mp3', filenum = 0, rm=True):
		...
		if audiofile.endswith('.wav'):
			...
			Saved_Audios_Path = os.path.dirname(os.path.realpath(__file__)) + '/saved'

			if os.system('lame --decode ' + audiofile + ' tmp.wav && sox -q tmp.wav -r ' + str(SAMPLE_RATE) + ' -c ' + str(CHANNELS) + ' -e signed-integer ' + Saved_Audios_Path + '/AUD_' + str(filenum) + '.wav') != 0:
				print "Can't process audio file... Check its a valid format"
			try:
				self.filename = Saved_Audios_Path + '/AUD_' + str(filenum) + '.wav'
