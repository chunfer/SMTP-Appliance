...
class SIPSession(object):
	...
	def __init__(self, ip, username, domain, password, auth_username=False, outbound_proxy=False, account_port=5060, display_name="-", timeout=DEFAULT_TIMEOUT):
		self.ip = ip
	...
	def reregister(self, register_string, register_frequency):
		...
		while self.listen_enabled:
			...
			timeouts_counter = 0
			while timeouts_counter < register_frequency:
				time.sleep(self.timeout)
				if not self.listen_enabled:
					break
	
				timeouts_counter += self.timeout
