...
class RTPHandler(object):
	...
	def __init__(self, ip, rtp_minport=DEF_RTPMINPORT, rtp_maxport=DEF_RTPMAXPORT):
		...
		self.transmission_timedout = False
	...
	def open(self):
		...
		#Set socket timeout
		self.rtp_socket.settimeout(SOCK_TIMEOUT)
		self.rtcp_socket.settimeout(SOCK_TIMEOUT)
	...
	def StartTransmission(self, ip, dport):
		...
		for n in range(int(DELAY / 0.02)):
			...
			#Accurate timer
			sleep_time = 0.02 - time.time() + self.start_time
	...
	def SendRTPPackages(self):
		...
		while self.sending_isenabled:
			try:
				...
				if eod:
					timeouts_counter += 1
					if timeouts_counter == MAX_TIMEOUT_COUNT:
						self.transmission_timedout = True 
	...
	def getTimeout(self):
		return self.transmission_timedout
