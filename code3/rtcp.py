...
class RTCPHandler(object):
	...
	def analyzeRTP(self, rtp_packet):
		#RTP packet analyzer
		Seq = int(rtp_packet[2:4].hex(), 16)
		Si = int(rtp_packet[4:8].hex(), 16)   
		SSRC = int(rtp_packet[8:12].hex(), 16)
	...
	def analyzeRTCP(self, rtcp_packet):
		...
		if PT == PT_SR:
			RC = rtcp_packet[0] & 31
			P = (rtcp_packet[0] & 32) >> 5
			length = int(rtcp_packet[2:4].hex(), 16) * 4
			SSRC = int(rtcp_packet[4:8].hex(), 16)
			...
			for n in range(RC):
				init_Rblock = 28 + n * 24
				SSRC_n = int(rtcp_packet[init_Rblock:init_Rblock + 4].hex(), 16)
				if SSRC_n == self.SSRC:
					jitter = int(rtcp_packet[init_Rblock + 12: init_Rblock + 16].hex(), 16)
					...
					print('SR')
					print('\tSSRC:', SSRC)
					print('\tfraction lost:', rtcp_packet[init_Rblock + 4])
					print('\tpackets lost:', int(rtcp_packet[init_Rblock + 5:init_Rblock + 8].hex(), 16))
					print('\tExtended Seq:', int(rtcp_packet[init_Rblock + 8:init_Rblock + 12].hex(), 16))
					print('\tjitter: ', jitter, 'lag:', self.lag)
					print('\tLSR(secs): ', int(rtcp_packet[init_Rblock + 16: init_Rblock + 18].hex(), 16))
			...
			for n in range(SC):
				SSRC = int(rtcp_packet[item_type_position - 4:item_type_position].hex(), 16)
				
				#analyze while there's no end package
				item_type = rtcp_packet[item_type_position]
				while item_type != END:
					item_length = rtcp_packet[item_type_position + 1]
					if item_type == CNAME:
						cname = rtcp_packet[item_type_position + 2: item_type_position + item_length + 3].decode()
						#Check for colisions
	...
	def genNTPbytes(self, NTP_time):
		...
		Lbytes_NTP = int((utc_time - Hbytes_NTP) / 0.5 * 2147483648)
	...
	def genReportBlock(self, SSRC, NTP_time):
		...
		fraction_lost = int(self.Rvars[SSRC][0] * 256 / packets_expected)
		...
		return struct.pack('!IIII', SSRC, lost_int, self.Rvars[SSRC][2], int(self.Rvars[SSRC][6])) + LSR + DLSR		
	...
	def genSDES(self):
		...
		header = struct.pack('!BBHI', V | 1, PT_SDES, int(length), self.SSRC)
		return header + items.encode()
	...
	def genBYE(self):
		...
		data = struct.pack('!BBHI', V | 1, PT_BYE, int(length), self.SSRC)
		if reason_length != 0:
			data += chr(reason_length).encode() + self.reason_bye.encode()

		return data

	def setreasonBYE(self, reason_bye):
		self.reason_bye = reason_bye
		self.send_bye = True
	
	def BYEwassent(self):
		return self.send_bye
