# -*- coding: utf-8 -*-
import sys, os, socket, re, random, hashlib, threading, time
...
class SIPSession(object):
	...
	def send_sip_bye(self, to_address):
		call_id = ''.join([random.choice('0123456789abcdef') for x in range(32)])

		bye_string = ""
		bye_string += "BYE sip:" + to_address + ":" + str(self.account_port) + " SIP/2.0\r\n"
		bye_string += "Via: SIP/2.0/UDP " + str(self.ip) + ":" + str(self.bind_port) + ";rport\r\n"
		bye_string += "Max-Forwards: 70\r\n"
		bye_string += "Contact: <sip:" + str(self.username) + "@" + str(self.ip) + ":" + str(self.bind_port) + ">\r\n"
		bye_string += 'To: <sip:' + to_address + ":" + str(self.account_port) + ">\r\n"
		bye_string += 'From: "' + str(self.display_name) + '"<sip:' + str(self.username) + "@" + str(self.domain) + ":" + str(self.account_port) + ">\r\n"
		bye_string += "Call-ID: " + str(call_id) + "\r\n"
		bye_string += "CSeq: 1 BYE\r\n"
		bye_string += "User-Agent: " + str(self.USER_AGENT) + "\r\n"
		bye_string += "Content-Length: 0\r\n"
		to_server = ""
		if self.outbound_proxy:
			to_server = self.outbound_proxy
		else:
		...
		self.sipsocket.sendto(bye_string.encode(), (to_server, self.account_port) )
		self.sip_history[call_id] = bye_string
		return call_id
	...
	def sip_listener(self):
		...
		try:
			...
			while self.stage == "WAITING":
				bin_data, addr = self.sipsocket.recvfrom(2048)				
				...
				if data.split("\r\n")[0] == "SIP/2.0 407 Proxy Authentication Required" or data.split("\r\n")[0] == "SIP/2.0 407 Proxy Authentication required":
					...
					self.sipsocket.sendto(ok.encode(), addr)
					...
					if data.startswith("CANCEL"):
						...
						self.sipsocket.sendto(terminated.encode(), addr)
	...
	def close(self):
		...
		self.stage = "BYE"
		self.kill_sipsocket.sendto('SIP session finished...'.encode(),(self.ip, self.bind_port))
		time.sleep(0.01)
